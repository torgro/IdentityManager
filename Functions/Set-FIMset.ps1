...
PROCESS {
    ...
    if(-not $SetObject)
    {
        $FIMobject = Get-FIMset -ObjectID $ObjectID @splat
        $FIMobject = $FIMobject.ResourceManagementObject
    }
    else
    {
        $FIMobject = $SetObject.ResourceManagementObject     
    }

    if($FIMobject.ObjectType -ne "Set")
    {
    ...
    { 
        [String]$NewFilter = New-FIMxpathFilter -xPath $XpathFilter
        if ($NewFilter)
        { 
        ...
        { 
            throw "New-FIMxPathFilter did not return an filter XML"
        }
    }

    Write-Verbose -Message "$F -  Create ImportObject of type Set"
    $importObject = New-FIMimportObject -ObjectType Set -ImportState Put
    
    $importObject.TargetObjectIdentifier = $FIMobject.ObjectID | ConvertTo-Guid
    ...
    foreach($key in $ChangeAttributes.Keys)
    {
        ...
        Write-Verbose "$f -  $key = $AttribValue"
        $change = New-FIMImportChange -AttributeName $key -AttributeValue $AttribValue -Operation Replace
    ...
    {
        if($RemoveExplicitMember)
        {
            $change = New-FIMImportChange -AttributeName ExplicitMember -AttributeValue $memb -Operation Delete
        }
        else
        {
            $change = New-FIMImportChange -AttributeName ExplicitMember -AttributeValue $memb -Operation Add
        }
