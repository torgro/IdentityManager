...
Param(
    ...
    ,
    [Parameter(ParameterSetName='BuildQuery')]
    [ValidateSet("*","Approval","Set","ObjectVisualizationConfiguration","spvOrganizationalUnit","ActivityInformationConfiguration","DetectedRuleEntry","Person","Group","Request","Resource","WorkflowDefinition","AttributeTypeDescription","BindingDescription","ObjectTypeDescription","SynchronizationRule","ManagementPolicyRule")]
    [string]$ResourceType
    ,
    [Parameter(ParameterSetName='BuildQuery')]
...
Process 
{
    ...
    if($PSBoundParameters.ContainsKey("Credential"))
    {
        ...
        [void]$splat.Add("Credential",$Credential)
    }
    ...
    if($ExportXpath -ne $null)
    {
        ...
        if($AttributeValue.Contains("*") -and $Attribute)
        {
            ...
            #if($AttributeValue.Length -eq 1)
            #{
            #    Write-Verbose -Message "$f -  Attribute value only contains '*', exporting all resourceTypes"
            #    $ExportXpath = "/$ResourceType"
            #}
